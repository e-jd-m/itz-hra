class Cell {
    ...
    check_surrounding(cells = []) {
        ...
        let top = cells[index(this.i, this.j - 1,this.cols,this.)]
        ...
        if (surrounding.length > 0) {
            let r = Math.floor(Math.random(0, surrounding.length));
...
function createMaze() {
    ...
    let rows;
    let cols;
    rows = Math.floor(800 / cell_r);
    cols = Math.floor(800 / cell_r);
